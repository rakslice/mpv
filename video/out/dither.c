...
typedef uint_fast32_t dither_index_t;

#define WRAP_SIZE2(k, x) ((dither_index_t)((dither_index_t)(x) & ((k)->size2 - 1)))
#define XY(k, x, y) ((dither_index_t)(((x) | ((y) << (k)->sizeb))))
...
struct ctx {
    ...
    dither_index_t randomat[MAX_SIZE2];
    bool calcmat[MAX_SIZE2];
    uint64_t gaussmat[MAX_SIZE2];
    dither_index_t unimat[MAX_SIZE2];
    AVLFG avlfg;
};
...
static void makegauss(struct ctx *k, unsigned int sizeb)
{
    ...
    for (dither_index_t c = 0; c < k->size2; c++)
        k->gauss[c] = 0;

    double sigma = -log(1.5 / UINT64_MAX * gauss_size2) / k->gauss_radius;

    for (dither_index_t gy = 0; gy <= k->gauss_radius; gy++) {
        for (dither_index_t gx = 0; gx <= gy; gx++) {
...
static void setbit(struct ctx *k, dither_index_t c)
{
    if (k->calcmat[c])
        return;
    k->calcmat[c] = true;
...
static dither_index_t getmin(struct ctx *k)
{
    uint64_t min = UINT64_MAX;
    dither_index_t resnum = 0;
    unsigned int size2 = k->size2;
    for (dither_index_t c = 0; c < size2; c++) {
        if (k->calcmat[c])
            continue;
...
void mp_make_fruit_dither_matrix(float *out_matrix, int size)
{
    ...
    for(dither_index_t y = 0; y < k->size; y++) {
        for(dither_index_t x = 0; x < k->size; x++)
...
{
    dither_index_t x = *(const dither_index_t *)a;
    dither_index_t y = *(const dither_index_t *)b;
    return x < y ? -1 : x > y;
}
